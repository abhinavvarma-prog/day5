...
print("=== Test 2: POST /api/webhook-test ===")
payload = {"message": "Hello from test script!", "project": "day5-vercel", "test": True}
resp = requests.post(f"{base_url}/api/webhook-test", json=payload)
print(f"Status: {resp.status_code}")
print(f"Response: {resp.json()}")
print()

# Test 3: Webhook POST with different data
print("=== Test 3: POST /api/webhook-test (different payload) ===")
payload2 = {"caller": "+1234567890", "event": "call_started", "timestamp": "2025-01-01T00:00:00Z"}
resp = requests.post(f"{base_url}/api/webhook-test", json=payload2)
print(f"Status: {resp.status_code}")
print(f"Response: {resp.json()}")
